...
uint8_t PCF8563_write(RtcDateTime* value) {
	...
	if (!I2C_Write(decToBcd(value->dayOfMonth))) {
		return 6;
	}

	if (!I2C_Write(decToBcd(value->dayOfWeek))) {
		return 7;
	}

	if (!I2C_Write(decToBcd(value->month))) {
		return 8;
	}

	if (!I2C_Write(decToBcd(value->year))) {
		return 9;
	}
...
uint8_t PCF8563_read(RtcDateTime *value) {
	...
	value->dayOfMonth = bcdToDec(I2C_read() & 0b00111111);
	I2C_sendAckBit();

	value->dayOfWeek = bcdToDec(I2C_read() & 0b00000111);
	I2C_sendAckBit();

	value->month = bcdToDec(I2C_read() & 0b00011111);
	I2C_sendAckBit();

	value->year = bcdToDec(I2C_read());
